...
def process(part, partition, exp_dir, frame_choice):
    ...
    frame_choice = [f for f in cellpose_files if str(frame_choice) in f]
    segmentation_choice = np.load(frame_choice[0], allow_pickle=True).item()['masks']
...
if __name__ == "__main__":
    ...
    if len(args) == 0:
        ...
        frame_choice = input("Enter a cellpose frame (ex. 001): ").strip()
        spawn(partitions, exp_dir, frame_choice)
    elif len(args) == 3:
        part, partitions, exp_dir = args
        frame_choice = input("Enter a cellpose frame (ex. 001): ").strip()
        process(int(part), int(partitions), exp_dir, frame_choice)
    elif len(args) >= 4:
        part, partitions, exp_dir, frame_choice = args[:4]
        process(int(part), int(partitions), exp_dir, frame_choice)
