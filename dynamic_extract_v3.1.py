...
def process_movie():
    global_path = "Clo_ctl1_temp_masks"
    exp_path = "/mnt/exDisk1/CalciumSignaling/Colo_GCAMP/Clofilium_ctl_1.2024-03-26-22-42-25"
    directory_path = f"{global_path}/processed_cells"

    image_files = sorted([f'{exp_path}/{f}' for f in os.listdir(exp_path) if f.endswith('.TIF')])
    ...
    cellpose_files.sort(key=sort_key)
